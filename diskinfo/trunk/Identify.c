	...
	if(dev == ATAPI_DEV) {
		...
		/* LBA addressing */
			...
			if( ((val[CMDS_SUPP_1] & VALID) == VALID_VAL) &&
				...
				bbbig = ((val[LBA_64_MSB]  * 65536.0 +
					  val[LBA_48_MSB]) * 65536.0 +
					  val[LBA_MID])    * 65536.0 +
					  val[LBA_LSB];
				printf("\tLBA48  user addressable sectors:%11.0lf\n",bbbig);
			}
		}
		if (!bbbig) bbbig = (__u64)(ll>mm ? ll : mm); /* # 512 byte blocks */
		if (!bbbig) bbbig = bb;
		printf("\tdevice size with Mi= 1024*1024: %11.0lf MiBytes\n",(bbbig * 512) / (1024 * 1024.0));
		bbbig = (bbbig * 512)/1000000;
		printf("\tdevice size with M = 1000*1000: %11.0lf MBytes ",bbbig);
		if(bbbig > 1000) printf("(%1.0lf GB)\n",bbbig/1000);
		else printf("\n");
